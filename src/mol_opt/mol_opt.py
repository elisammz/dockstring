...
def _gfr_v1(score_dict):
    """
    Scoring function for GFR inhibitor design.

    Args:
        score_dict (dict): Dictionary containing docking scores for FGFR1, EGFR, and KDR.

    Returns:
        float: Composite score balancing potent FGFR1 binding and off-target penalties.
    """
    fgfr1 = score_dict["FGFR1"]
    egfr = score_dict["EGFR"]
    kdr = score_dict["KDR"]
    ...
    composite_score = fgfr1 - egfr_penalty - kdr_penalty
...
def get_cached_objective_and_dataframe(
    ...
    elif objective_name == "GFR":
        for protein in ["EGFR", "FGFR1", "KDR"]:
            items_to_calculate[protein] = _get_safe_dock_function(
                protein, **dock_kwargs
            )
        scalar_fn = _gfr_v1
