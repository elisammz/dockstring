...
def prepare_dataset(dataset: pd.DataFrame) -> pd.DataFrame:
    ...
    return dataset[dataset["target"].apply(lambda t: t in targets)].copy()
...
def main():
    ...
    for prop in ["ef", "auc", "ap"]:
        fig_width = 6.50128  # inches
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(fig_width, 1.5), constrained_layout=True)
        ...
        for i, (offset, column) in enumerate(zip([-1, 0, 1], ["qed", "logp", "score"])):
            ax.bar(
                ...
                width,
                color=colors[i],
                label=metric_label_dict[column],
                align="center",
            )
